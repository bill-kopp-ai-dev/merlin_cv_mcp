...
def extract_video_frames_tool(
    ...
    """
    Extrai quadros (frames) de um vídeo em intervalos específicos salvando-os como imagens.
    
    PIPELINE E USO ESTRATÉGICO:
    - OBRIGATÓRIO ANTES de enviar um vídeo para um LLM de Visão. A maioria dos modelos só lê imagens.
    - Se o usuário perguntar "o que acontece neste vídeo?", extraia os frames a cada X segundos e analise as imagens.
    
    REGRAS DE ARQUIVO:
    - 'video_path': Caminho do vídeo no workspace.
    - A pasta de extração ('output_dir') será automaticamente gerada se não informada.
    """
    try:
        video_path = str(safe_path(video_path))
        if output_dir is not None:
            output_dir = str(safe_path(output_dir))
        # Validate video file
...
def detect_motion_tool(
    ...
    """
    Detecta movimento/diferença visual significativa entre DOIS quadros (frames) extraídos.
    
    PIPELINE E USO ESTRATÉGICO:
    - Excelente para economizar chamadas LLM. Se você extraiu vários quadros de uma câmera de segurança,
      use isso para ver onde houve movimento antes de enviar o quadro a um modelo caro como o Percival.
      
    REGRAS DE ARQUIVO:
    - 'frame1_path' e 'frame2_path': Caminhos das imagens extraídas no workspace.
    """
    try:
        frame1_path = str(safe_path(frame1_path))
        frame2_path = str(safe_path(frame2_path))
...
def track_object_tool(
    ...
    """
    Acompanha o movimento de um objeto específico ao longo dos quadros de um vídeo a partir de uma área inicial (bbox).
    
    PIPELINE E USO ESTRATÉGICO:
    - Raramente usado de forma autônoma sem supervisão. Depende do usuário passar o bounding box inicial.
    - Útil se a tarefa for "extrair o trajeto deste objeto X".
    
    REGRAS DE ARQUIVO:
    - 'video_path': Caminho do vídeo.
    - initial_bbox é MANDATÓRIO neste nível.
    """
    try:
        video_path = str(safe_path(video_path))
        if output_dir is not None:
        ...
        if not success:
            raise ValueError("Failed to initialize tracker with the provided bounding box")
...
def combine_frames_to_video_tool(
    ...
    """
    Agrupa uma sequência de imagens estáticas gerando um único arquivo de vídeo.
    
    PIPELINE E USO ESTRATÉGICO:
    - Caso você tenha extraído frames, modificado-os (desenhado caixas, filtrado) e 
      queira devolver um vídeo final anotado para o usuário.
      
    REGRAS DE ARQUIVO:
    - 'frame_paths': Lista de imagens.
...
def create_mp4_from_video_tool(
    ...
    """
    Converte um vídeo original para o formato MP4 padronizado, podendo ajustar a resolução/qualidade.
    
    PIPELINE E USO ESTRATÉGICO:
    - Crucial se o vídeo original for muito pesado (4K) ou se o formato não for bem suportado para leitura.
    - Reduzir a resolução antes do processamento acelera absurdamente todo o pipeline seguinte.
...
def detect_video_objects_tool(
    ...
    """
    Usa DNN/YOLO para encontrar objetos ao longo de um vídeo e gera um NOVO VÍDEO com as caixas (bounding boxes) desenhadas.
    
    PIPELINE E USO ESTRATÉGICO:
    - Processamento completo e imediato se a tarefa for "diga quantos carros passam neste vídeo".
    - Pode ser processual e demorado para vídeos longos. Use em amostras pequenas.
...
def detect_camera_objects_tool(
    ...
    """
    Liga a webcam padrão, detecta objetos ao vivo via DNN/YOLO e salva a gravação.
    
    PIPELINE E USO ESTRATÉGICO:
    - Somente deve ser invocado mediante comando explícito do usuário como "ligue a câmera e grave o que ver".
    - Atenção: Requer acesso físico ao hardware. Pode falhar se estiver num servidor isolado (docker sem binding v4l2).
