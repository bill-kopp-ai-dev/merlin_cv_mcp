...
def apply_filter_tool(
    ...
    """
    Aplica filtros de desfoque (blur, gaussian, median, bilateral) em uma imagem.
    
    PIPELINE E USO ESTRATÉGICO:
    - Use esta ferramenta para reduzir ruídos ou suavizar detalhes ANTES de aplicar detecção de bordas (edges).
    - RARAMENTE útil antes de enviar para LLMs de visão, a menos que a imagem tenha muito granulado atrapalhando o OCR.
    
    REGRAS DE ARQUIVO:
    - 'image_path': Caminho da imagem no workspace.
    - O arquivo filtrado será salvo automaticamente com um sufixo apropriado.
    """
    try:
...
def detect_edges_tool(
    ...
    """
    Detecta os contornos e bordas de uma imagem (canny, sobel, laplacian, scharr).
    
    PIPELINE E USO ESTRATÉGICO:
    - Excelente PREPARAÇÃO para extrair a estrutura de objetos ou ler formas arquitetônicas sem a distração das texturas.
    - Útil se o usuário pedir para analisar apenas o "esqueleto" ou "rascunho" de um objeto.
    
    REGRAS DE ARQUIVO:
    - 'image_path': Caminho da imagem no workspace. A saída será salva no mesmo diretório.
    """
    try:
        image_path = str(safe_path(image_path))
        # Read image from path
...
def apply_threshold_tool(
    ...
    """
    Aplica limiarização (threshold), convertendo a imagem em pixels estritamente pretos ou brancos.
    
    PIPELINE E USO ESTRATÉGICO:
    - FORTE ALIADO do OCR (leitura de texto). Se o texto estiver com baixo contraste, 
      aplique 'adaptive' threshold ANTES de enviar para um LLM de visão ler o texto.
    
    REGRAS DE ARQUIVO:
    - 'image_path': Caminho da imagem no workspace. A conversão binária será salva com um sufixo.
    """
    try:
        image_path = str(safe_path(image_path))
        # Read image from path
...
def detect_contours_tool(
    ...
    """
    Detecta e desenha linhas de contorno em uma imagem (vetorização lógica).
    
    PIPELINE E USO ESTRATÉGICO:
    - Diferente da detecção de bordas, esta ferramenta retorna coordenadas lógicas e desenha ativamente na imagem.
    - Útil para contar objetos ou destacar separações corporais ANTES da análise visual.
    
    REGRAS DE ARQUIVO:
    - 'image_path': Caminho da imagem no workspace. Salva a imagem com os contornos destacados.
    """
    try:
        image_path = str(safe_path(image_path))
        # Read image from path
...
def find_shapes_tool(
    ...
    """
    Encontra e desenha formas geométricas básicas (círculos, linhas).
    
    PIPELINE E USO ESTRATÉGICO:
    - Use para automatizar contagens de objetos redondos ou alinhar detecção de retas.
    - O retorno em JSON pode responder diretamente a perguntas do usuário como "quantos círculos existem",
      dispensando o LLM de visão em imagens simples.
      
    REGRAS DE ARQUIVO:
    - 'image_path': Caminho da imagem. Imagem anotada será gerada.
    """
    try:
        image_path = str(safe_path(image_path))
        # Read image from path
...
def match_template_tool(
    ...
    """
    Compara e encontra a localização exata de um "recorte" (template) dentro de uma imagem maior.
    
    PIPELINE E USO ESTRATÉGICO:
    - Use esta ferramenta quando o usuário fornecer DUAS imagens: a imagem original e um pequeno fragmento
      que deve ser localizado nela.
    - Retorna as coordenadas exatas da localização e gera uma imagem com a área destacada.
    
    REGRAS DE ARQUIVO:
    - 'image_path': A imagem grande (onde procurar).
    - 'template_path': A imagem pequena (o que procurar).
    """
    try:
        image_path = str(safe_path(image_path))
        template_path = str(safe_path(template_path))
